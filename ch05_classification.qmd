...
Define a recipe to convert categorical inputs and outputs to factor type.

```{r}
nb_rec <- 
  recipe(class ~ gender + age_gr, data = dat3) |> 
  step_num2factor(class, levels = c("1", "2")) |> 
  step_num2factor(age_gr, levels = c("1", "2", "3", "4")) |> 
  step_string2factor(gender)
```

Then, train the recipe by calling `prep()` function.

```{r}
nb_rec_fit <- prep(nb_rec)
```

Let us create training data to feed to modeling step, by calling `juice()` function.

```{r}
training_dat <- juice(nb_rec_fit)
training_dat
```
...
nb_fit <- 
  nb_model |> 
  fit(class ~ gender + age_gr, data = training_dat)
...
```{r}
results <- training_dat |> 
  bind_cols(
    predict(nb_fit, new_data = training_dat, type = "prob"),
    predict(nb_fit, new_data = training_dat, type = "class")
  )
...
```{r}
results |> 
  multiple_metrics(truth = class, estimate = .pred_class)
```
